  ...
  return async (
    ...
    try {
      ...
      if (
        ...
        let state: ExtendedResponseJSON<ResponsePayload> = initialData;
        ...
        while (
          ...
          !lastResults['']?.isComplete
        ) {
          state = {
            ...lastResults[''],
            data:
              updateState?.(state.data, lastResults[''].data) ||
              lastResults[''].data,
          };
          const getDataWithRouteLabels = {
            '': getData,
            ...getAdditionalStreams?.(reqPayload, state),
          };
