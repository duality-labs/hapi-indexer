...
import bankReservesDeltasTimeseries, {
  bankReservesDeltasAtHeightTimeseries,
} from './bankReservesDeltas';
...
export function bankReservesAtHeightTimeseries(
  address: string,
  denom0: string,
  denom1: string
) {
  return sql`
    WITH
      bank_balance_deltas_union AS (${bankReservesDeltasAtHeightTimeseries(
        address,
        denom0,
        denom1
      )})
