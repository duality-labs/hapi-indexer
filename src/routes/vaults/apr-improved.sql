...
slinky_price_ids AS (
            ...
            FROM vault_configs as v
            ANY LEFT JOIN spacebox.slinky_pairs_state as p_0
              ON v."token_0_symbol" = p_0."base"
             AND v."token_0_quote_currency" = p_0."quote"
            ANY LEFT JOIN spacebox.slinky_pairs_state as p_1
              ON v."token_1_symbol" = p_1."base"
             AND v."token_1_quote_currency" = p_1."quote"
          ),
          slinky_prices_0 AS (
            SELECT *
            FROM spacebox.slinky_prices
            WHERE "id" IN (SELECT "price_id_0" FROM slinky_price_ids)
          ),
          slinky_prices_1 AS (
            SELECT *
            FROM spacebox.slinky_prices
            WHERE "id" IN (SELECT "price_id_1" FROM slinky_price_ids)
          ),
