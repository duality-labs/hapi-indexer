        WITH
          1 as "timePeriods",
          100 as "timePeriodLimit",
          0 as "unix_from",
          0 as "unix_to",
          'neutron19legwvmazmk5cxx66pp6h9fh82k3lycrqg96nsud4h3cmzs6dkrqdqvke6' as "_contract_address",
          'neutron1e2c5p8y5rw2hp4fjr05uvkrkz76ej0kqegnwxe' as "_user_address",
          if(
            "unix_to" > 0,
            toDateTime("unix_to"),
            toStartOfInterval(
              addMinutes(now(), -10),
              INTERVAL "timePeriods" DAY
            )
          ) as "time_end",
          greatest(
            if (
            ...
            subDate(
              "time_end",
              INTERVAL ("timePeriods" * "timePeriodLimit") DAY
            )
          ) as "time_start",
          COALESCE(
            (SELECT "height"
          ...
          time_range AS (
            SELECT
              ...
              FROM generate_series(
                0,
                dateDiff(DAY, "time_start", "time_end")
              )
          ),
          slinky_prices_0 AS (
            SELECT *
          ...
          timeseries AS (
            ...
            SELECT
              ...
              subDate(t."timestamp", INTERVAL 1 DAY) as "time",
