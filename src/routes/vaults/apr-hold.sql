...
balance_adjustment_rows AS (
  WITH
    ...
    SELECT
      ...
      b."token_0_withdrawn" as "token_0_withdrawn",
      b."token_1_withdrawn" as "token_1_withdrawn",
      "price_id_0",
      "price_id_1"
...
balance_hold_value AS (
  WITH
    (SELECT "token_0_decimals" FROM token_config) as "token_decimals_0",
    (SELECT "token_1_decimals" FROM token_config) as "token_decimals_1",
    ...
    COALESCE(p0."price", "first_price_0") as "slinky_price_0",
    COALESCE(p1."price", "first_price_1") as "slinky_price_1",
    ...
    -- find the withdrawal deltas by ordering the adjustments by sort_key and multiplying
    -- and using: r.token0_vault * (o.shares_action_amount / (o.shares_total+o.shares_action_amount))

  SELECT
    b."timestamp" as "timestamp",
    b."height" as "height",
    "value" / 2 / "token_price_0" as "hold_amount_0",
    "value" / 2 / "token_price_1" as "hold_amount_1"
  FROM balance_adjustment_rows as b
